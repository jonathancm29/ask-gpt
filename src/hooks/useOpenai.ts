...
const useOpenai = () => {
  ...
  const askGPT = async (apiKey: string, { prompt, model, temperature }: {prompt: string, model: string, temperature: number}) => {
    setError('')
    setResponseAI('')
    ...
    try {
      setLoading(true)
      const res = await query(apiKey, { prompt, model, temperature })
      if (!res.success) {
        setError(res.error)
      }
